...
def get_false_postive_bags(trained_model, train_dl):
    ...
    for batch_id, (data, label) in enumerate(train_dl):
        ...
        data, bag_label = torch.autograd.Variable(data), torch.autograd.Variable(
            bag_label
        )

        _, predicted_label, attention_weights = trained_model.forward(
            data
        ...
        if predicted_label == bag_label:
            ...
            false_positive_bags.append(
                data.squeeze()
            )  # remove the batch/bag dimension and add all tiles of the bags as (num_tiles, channel, H, W)
...
def determine_hard_negative_instances(false_positive_bags, attention_weights):
    ...
    for bag, attention_vector in zip(false_positive_bags, attention_weights):
        ...
        H_bag = [
            bag_tile
            for bag_tile, attention_weight in zip(bag, attention_vector)
            if attention_weight >= (mean_attention + std_attention / 5)
        ]
    ...
    hard_negative_instances = [
        instance for sublist in hard_negative_instances for instance in sublist
    ]
...
def new_bag_generation(hard_negative_instances, training_ds, n_clusters=10):
    ...
    FeatureExtractor.classifier = FeatureExtractor.classifier[
        :-1
    ]  # drop last layer (classification)
    ...
    assert FeatureExtractor(torch.rand((1, 3, 224, 224))).size() == torch.Size(
        [1, 4096]
    )  # assert model output
    # model done
    ...
    for instance in hard_negative_instances:
        ...
        instance_input = torch.cat(
            (instance, instance, instance), dim=0
        )  # achieve faux-RGB
        # instance_input = instance_input[:, 63:-63, 63:-63] # crop 350x350 to 224x224
        feature_vector = FeatureExtractor(
            instance_input.unsqueeze(dim=0)
        )  # produces [1,4096] torch tensor
    ...
    cluster_labels = KMeans(n_clusters=n_clusters, random_state=1337).fit_predict(
        all_feature_vectors
    )  # assigns cluster label for each sample == for each hard negative instance feature vector
    ...
    for i in range(n_clusters):
        ...
        for sample in range(new_bag_size):
            chosen_cluster = np.random.choice(
                cluster_labels, size=1, replace=False, p=None
            )  # get a random cluster
            chosen_instances = np.array(hard_negative_instances)[
                np.where(cluster_labels == chosen_cluster)[0]
            ]  # get all hard negative instances assigned to this kMeans cluster
            new_instance = chosen_instances[
                np.random.choice(
                    chosen_instances.shape[0], size=1, replace=False, p=None
                ),
                ...,
...
def add_back_to_dataset(training_ds, new_bags):
    ...
    for bag in new_bags:
        numpy_bag = [
            torch_tensor_cuda.unsqueeze(dim=0).cpu().numpy()
            for torch_tensor_cuda in bag
        ]
        new_label = [
            [0]
        ]  # [0,0,1] # (or 0) making it 'normal' or 'negative' in multiclass or binary setting
