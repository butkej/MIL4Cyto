...
def parse_args():
    ...
    parser.add_argument('--use_max', dest='use_max',
    ...
    parser.add_argument('--use_adaptive', dest='use_adaptive',
...
def preprocess_data(data, labels, input_dim):
    ...
    for wsi in data:
        ...
        centroids = preprocessing.find_contours_and_centroids(thresh, morph_kernel=(10,10), lower_bound_area=900, upper_bound_area=5000)
        patch_collection = preprocessing.blowup_patches(wsi_gray, centroids, patch_size=input_dim, multichannel=False)
        patch_collection = preprocessing.filter_patches(patch_collection, crop=50, factor=1.025)
...
def choose_optimizer(selection, model):
    ...
    if str(selection) == 'adam':
        ...
        optim = torch.optim.Adadelta(model.parameters(), lr=1.0, rho=0.9, eps=1e-06, weight_decay=0)
    elif str(selection) =='momentum':
        optim = torch.optim.SGD(model.parameters(), lr=0.01, momentum=0.9, dampening=0, weight_decay=0, nesterov=False)
    else:
...
def fit(model, optim, train_dl, validation_dl, model_savepath):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print('Epoch [{}] : Train Loss {:.4f}, Train Acc {:.4f}, Val Loss {:.4f}, Val Acc {:.4f}'.format(epoch, result['train_loss'], result['train_acc'], result['val_loss'], result['val_acc']))
...
def k_fold_cross_val(X, y, og_labels, model, optim, metric_savepath, model_savepath, splits):
    ...
    for train_index, val_index in KF.split(x, og_labels):
        ...
        print('Ratio of {} negative to {} positive cases.'.format(len([int(i[1][0]) for i in training_ds if int(i[1][0]) == 0]), len([int(i[1][0]) for i in training_ds if int(i[1][0]) == 1])))
        print('\nValidation DS contains {} patients'.format(len(validation_ds)))
        print('with labels: ' + str([int(i[1][0]) for i in validation_ds]))
        print('Ratio of {} negative to {} positive cases.'.format(len([int(i[1][0]) for i in validation_ds if int(i[1][0]) == 0]), len([int(i[1][0]) for i in validation_ds if int(i[1][0]) == 1])))
        
        # random minority oversampling
        while len([row[1] for row in training_ds if row[1][0]==0]) < len([row[1] for row in training_ds if row[1][0]==1]):
        ...
        # Data Generators
        train_dl = torch.utils.data.DataLoader(training_ds, batch_size=1, shuffle=True, **loader_kwargs)
        validation_dl = torch.utils.data.DataLoader(validation_ds, batch_size=1, shuffle=False, **loader_kwargs)
        ...
        mil_metrics.plot_conf_matrix(y_true, y_pred, metric_savepath,
        ...
        false_positive_bags, attention_weights_list = mining.get_false_postive_bags(model, train_dl)
        print('-------------------------------------------------')
        print('Hard Negative Mining...')
        hard_negative_instances = mining.determine_hard_negative_instances(false_positive_bags, attention_weights_list)
        if not len(hard_negative_instances):
            print('No hard negative instances found!')
        else:
            new_bags = mining.new_bag_generation(hard_negative_instances, training_ds, n_clusters=4)    
        ...
        metric_savepath = metric_savepath + str('_afterHNM') # modify savepath to reflect result changes after HNM
        model_savepath = model_savepath + str('_afterHNM') # modify savepath to reflect result changes after HNM
...
if __name__ == "__main__":
    ...
    input_dim = (150,150) # size of a single tile and also network input dimensionality
    ...
    # Data
    print('Loading data ...') # in this case there were 2 datasets, one for normal and one for cancerous samples
    ...
    k_fold_cross_val(X, y, og_labels, model, optim, metric_savepath, model_savepath, splits=3)
